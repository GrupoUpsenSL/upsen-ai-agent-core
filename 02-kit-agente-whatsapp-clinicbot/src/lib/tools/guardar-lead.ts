...
import { upsertLead as upsertLeadHighlevel, highlevelConfigured } from "../highlevel";
import { rememberLead, getLeadMemory } from "../memory";

// Registra el lead en el CRM (Airtable y/o HighLevel).
...
export const guardarLeadHandler: ToolHandler<GuardarLeadArgs> = async (args) => {
  ...
  // 2) CRM de ventas: SOLO se registra cuando ya hay nombre Y email.
  ...
  if (!airtableConfigured() && !highlevelConfigured()) {
    return { ok: false, message: "CRM no configurado. Añade AIRTABLE_* o HIGHLEVEL_* en .env.local." };
  }
  ...
  // Airtable y HighLevel se intentan de forma independiente: un fallo en uno
  // NO debe impedir el otro ni, sobre todo, nunca debe tumbar la respuesta
  // del agente (por eso cada llamada va en su propio try/catch aquí, además
  // de que ambos módulos ya capturan sus propios errores internamente).
  ...
  if (airtableConfigured()) {
    try {
      ...
      results.push(`Airtable: ${res.message}`);
    } catch (err) {
      results.push(`Airtable: error inesperado (${err instanceof Error ? err.message : String(err)})`);
    }
  }

  if (highlevelConfigured()) {
    try {
      const res = await upsertLeadHighlevel(phone, {
        nombre: nombreFinal,
        email: emailFinal,
        negocio: (objetivo || mem?.objetivo) as string | undefined,
        necesidad: notas,
      });
      if (res.ok) anyOk = true;
      results.push(`HighLevel: ${res.message}`);
    } catch (err) {
      results.push(`HighLevel: error inesperado (${err instanceof Error ? err.message : String(err)})`);
    }
  }
  ...
  return {
    ok: anyOk,
    guardadoEnCRM: anyOk,
    message: (anyOk ? "Lead registrado en el CRM (nombre + email). " : "") + results.join(" · ") + emailAviso,
    ...(emailAviso ? { pedirEmail: true } : {}),
  };
};