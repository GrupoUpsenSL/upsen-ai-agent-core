...
/**
 * Ruta al fichero de negocio a usar. Si AGENT_CONFIG está definida (p.ej.
 * "upsenai"), carga prompts/negocio.<AGENT_CONFIG>.md; si no, carga el
 * prompts/negocio.md de siempre. Es una FUNCIÓN (no una constante de
 * módulo) para leer process.env en el momento de la llamada, no al importar
 * — evita el mismo problema de "hoisting" de ES modules que ya documenta
 * env-loader.ts.
 *
 * Esto es lo que permite correr una SEGUNDA instancia del mismo código con
 * otra configuración de negocio: cada instancia vive en su propia carpeta
 * con su propio .env.local, y basta con poner AGENT_CONFIG=<nombre> ahí.
 * Ver README → "Añadir una tercera instancia" para el procedimiento completo.
 */
function negocioPath(): string {
  const agentConfig = process.env.AGENT_CONFIG?.trim();
  const filename = agentConfig ? `negocio.${agentConfig}.md` : "negocio.md";
  return path.resolve(process.cwd(), "prompts", filename);
}
...
export function buildSystemPrompt(memoryContext = ""): string {
  const rutaNegocio = negocioPath();
  if (!fs.existsSync(rutaNegocio)) {
    return FALLBACK_PROMPT;
  }

  const negocio = fs.readFileSync(rutaNegocio, "utf-8");
