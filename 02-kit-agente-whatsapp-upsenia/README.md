...
## Ejecutar varias instancias (multi-cliente)

Este kit permite correr varias instancias independientes del agente desde
el mismo código — una por cliente — cada una con su propia configuración
de negocio, su propio pipeline de CRM, su propio puerto, su propia sesión
de WhatsApp y su propio historial de conversaciones. Ahora mismo hay dos
instancias corriendo como referencia: ClinicBot (puerto 3000) y UPSEN AI
(puerto 3001).

Cada instancia es una **copia separada de toda la carpeta del proyecto**.
Esto es intencional, no una limitación: como la base de datos SQLite
(`data/`), la sesión de WhatsApp (`auth/`) y la configuración (`.env.local`)
se resuelven relativas a la carpeta desde la que corre el proceso, darle a
cada cliente su propia carpeta garantiza aislamiento total — que una
instancia se caiga, se quede sin créditos de la API, o pierda la sesión de
WhatsApp nunca puede afectar a otra.

### Para añadir una nueva instancia (p. ej. un tercer cliente)

1. **Duplica la carpeta del proyecto.**
```bash
   cp -r 02-kit-agente-whatsapp 02-kit-agente-whatsapp-<cliente>
   cd 02-kit-agente-whatsapp-<cliente>
```

2. **Borra el estado propio de la instancia** para que arranque limpia (NO
   copies las conversaciones ni la sesión de WhatsApp de otro cliente):
```bash
   rm -rf data auth
```

3. **Añade el fichero de negocio** en `prompts/negocio.<nombre>.md`, donde
   `<nombre>` coincide con el valor de `AGENT_CONFIG` que pondrás en el
   paso 4 (p. ej. `prompts/negocio.acme.md` para `AGENT_CONFIG=acme`).
   Básalo en `prompts/negocio.example.md` o en el fichero de un cliente ya
   existente como plantilla.

4. **Crea `.env.local`** (copia `.env.example` como punto de partida) y
   rellena, como mínimo:

AGENT_CONFIG=<nombre> # debe coincidir con el fichero del paso 3
PORT=<puerto-libre> # p. ej. 3002 para una tercera instancia
OPENROUTER_API_KEY=... # puedes reutilizar una key existente o usar una nueva
HIGHLEVEL_API_KEY=... # misma sub-cuenta de HighLevel u otra nueva
HIGHLEVEL_LOCATION_ID=...
HIGHLEVEL_PIPELINE_ID=... # el pipeline de ESTE cliente — nunca reutilices el de otro
HIGHLEVEL_STAGE_ID=...
HIGHLEVEL_LEAD_TAG=<nombre>-lead # una etiqueta única para este cliente
ALLOWED_PRICES=... # los precios reales de este cliente (guardrail)
ALLOWED_HOSTS=<dominio-cliente> # el/los dominio(s) propio(s) de este cliente
SECURITY_CANARY=... # genera un valor nuevo y único

   `SUPABASE_URL`/`SUPABASE_SERVICE_KEY` se pueden compartir entre
   instancias sin problema si usas memoria de largo plazo — se indexa por
   teléfono/sesión, así que los leads de distintos clientes no se
   mezclan.

5. **Monta el pipeline de HighLevel del cliente** (mismo proceso que en la
   Task S3): crea un pipeline con al menos 2 etapas, una etiqueta, y los
   campos personalizados que necesite el agente — luego usa esos IDs
   reales en el paso 4.

6. **Actualiza la página de demo**, `public/demo.html`, con la marca de
   este cliente (nombre del agente, color, texto de bienvenida) en los
   atributos `data-*` de la etiqueta `<script>`. `public/widget.js` en sí
   **no necesita ningún cambio** — está pensado para configurarse entero
   por fuera.
...
8. **Arranca — ojo, `PORT` tiene que ir como variable real del shell, no
   solo en `.env.local`** (la CLI de Next.js lee el puerto antes de que
   se cargue su propio `.env.local`):
```bash
   PORT=<puerto-libre> npm run start:all
```

9. **Prueba:** abre `http://localhost:<puerto-libre>/demo.html`, manda un
   saludo, haz una pregunta específica de los servicios de este cliente,
   y manda un mensaje con nombre + email para confirmar que el lead llega
   al pipeline correcto de HighLevel con la etiqueta correcta.

10. **Confirma el aislamiento:** con esta instancia corriendo, comprueba
    que el resto de instancias (ClinicBot, UPSEN AI, etc.) siguen
    respondiendo con normalidad en su propia terminal/puerto — nunca
    deberían interferir entre sí.

### Limitación conocida

Los leads del canal web (vía el widget de chat) usan un identificador
interno `web:<sessionId>` en vez de un teléfono real. `highlevel.ts` ya
lo contempla: omite el campo `phone` y usa el `email` como clave del
upsert para los leads de web — los leads de WhatsApp no se ven afectados
y siguen haciendo upsert por teléfono con normalidad.